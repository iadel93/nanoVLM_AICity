...
class VQACollator(object):  # Visual Question Answering Collator
    ...
    def __call__(self, batch):
        # Support VQADataset-style input
        images = [item["images"] if "images" in item else item["image"] for item in batch]
        texts = [item["texts"]["user"] if "texts" in item else item["text_data"] for item in batch]
        answers = [item["texts"]["assistant"] if "texts" in item else item["answer"] for item in batch]
